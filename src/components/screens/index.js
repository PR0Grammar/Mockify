/**
 * @returns the screens for the navigators
 * 
 * @desc 
 * Each screen is basically a 'container' of all its components.
 * It should only deal with displaying and navigating EXCLUDING the SplashPage
 */

export {default as Home } from './Home';
export {default as Browse } from './Browse';
export {default as Radio } from './Radio';
export {default as Search } from './Search';
export {default as YourLibrary } from './YourLibrary';
export {default as Login } from './Login';
export {default as InitialLandingPage} from './InitialLandingPage';
export {default as SplashPage} from './SplashPage';
export {default as AlbumSongList} from './AlbumSongList';
